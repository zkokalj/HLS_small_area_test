...
#For images.txt : Landsat 7 / Landsat 8
...
#List of bands
list_band = [ ["_sr_band1.tif", "_sr_band2.tif"], ["_sr_band2.tif", "_sr_band3.tif"], ["_sr_band3.tif", "_sr_band4.tif"], ["_sr_band4.tif", "_sr_band5.tif"], ["_sr_band5.tif", "_sr_band6.tif"], ["_sr_band7.tif", "_sr_band7.tif"]]
...
number_pixels = [0 for i in range(6)]
difference_value = [0.0 for i in range(6)]
difference_percentage = [0.0 for i in range(6)]

for comparison in images_txt:
    #Get names of image folder
    list_images = comparison.split(";")
    folder_L8 = list_images[1].strip()
    folder_L7 = list_images[0].strip()
    print(folder_L7)
    ...
    emprise_L7, emprise_L8, transform = intersection(dataset_Fmask_L7, dataset_Fmask_L8)
    ...
    for num_band in range(len(list_band)):#For each band
        ...
        #Open images
        image_path_L7 = os.path.join(input_L7, folder_L7, folder_L7+band[0])
        ...
        image_L7 = image_L7 / 10000

        
        #Modify spectral band to avoid zero
        masque = np.ones((image_L7.shape))
        masque[image_L7==0]=0
        image_L8_bis = image_L8
        image_L7_bis = image_L7
        image_L8_bis[masque==0]=10
        image_L7_bis[masque==0]=10

        #Calculate difference
        difference = np.abs(image_L8 - image_L7)
        difference_pourcentage = np.abs(image_L8_bis - image_L7_bis) / np.abs(image_L7_bis) * 100

        #Apply masks
        difference_mask =  difference * mask
        difference_pourcentage_masque = difference_pourcentage * mask
        if not os.path.exists(os.path.join(test, folder_L7)):
            os.makedirs(os.path.join(test, folder_L7))
        save(os.path.join(test, folder_L7, band[0]+"_difference_percentage.tif"), difference_pourcentage_masque, transform, np.float32, 0)
