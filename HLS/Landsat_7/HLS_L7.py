...
"""
Process HLS algorithm on Landsat-8 images. One directory is mandatory : result of LaSRC algorithm. One xlsx file is required : Landsat Collection 1 to Collection 2 PR Mean Offsets (https://www.usgs.gov/media/files/landsat-collection-1-collection-2-pr-mean-offsets)
"""

#Path of LEDAPS result
input_LEDAPS_result = "Ledaps_result"
...
f_iso = [0.774, 0.1306, 0.1690, 0.3093, 0.3430, 0.2658]
f_geo = [0.079, 0.0178, 0.0227, 0.0330, 0.0453, 0.0387]
...
mask_data = lagrange(x, y)
mask_data=list(Polynomial(mask_data).coef)
mask_data.reverse()
...
#Band in Landsat 7 images
list_band = ["_Fmask4.tif", "_sr_band1.tif", "_sr_band2.tif", "_sr_band3.tif", "_sr_band4.tif","_sr_band5.tif",  "_bt_band6.tif","_sr_band7.tif"]

#Spectral bands processed by BRDF normalization
list_band_brdf = ["_sr_band1.tif", "_sr_band2.tif", "_sr_band3.tif", "_sr_band4.tif", "_sr_band5.tif",  "_sr_band7.tif"]

#Spectral bands not processed by BRDF normalization
list_band_without_brdf = ["_bt_band6.tif"]


#Offset and slope for band adjustment
table_band_adjustment = [["1", -0.0095, 0.9785], ["2", -0.0016, 0.9542], ["3", -0.0022, 0.9825], ["4", -0.0021, 1.0073], ["5", -0.0030, 1.0171], ["7", 0.0029, 0.9949]]
...
def process_Fmask(path_file, file, directory_main, dst_transform):
    ...
    if os.path.exists(path_Fmask):
        ...
        mask_no_data = nppol.polyval(Fmask, mask_data)
        mask_no_data = (mask_no_data+0.1).astype("i8")
        Fmask3 = (channel_small_values - 6) * mask_no_data + 6
        ...
        #Convolution
        resample = ndimage.convolve(Fmask3, window)
...
def coregistration(directory_LaSRC, directory_workspace, translation_latitude, translation_longitude, file):
    """
    Process coregistration on image and return rasterio.Affine object for final position of L30 images.
    """
    for name in list_band:
        dataset = rasterio.open(os.path.join(directory_LaSRC, file+name))
        ...
        if name != "_Fmask4.tif":
            path_output = os.path.join(directory_workspace, file+name+"_intermediaire.tif")
            nan_array = nan_array / 10000
        else:
            path_output = os.path.join(directory_workspace, file+name)
        #Save spectral band
        with rasterio.open(
                path_output,
                'w',
...
def BRDF(directory_workspace, num_band, file, K_geo_sensor, K_geo_norm, K_vol_sensor, K_vol_norm, directory_main):
    ...
    if os.path.exists(path_band):
        ...
        BRDF_image = BRDF_image * table_band_adjustment[num_band][2] + table_band_adjustment[num_band][1]
...
def create_xml_metadata(directory_LaSRC, file, directory_main, solar_azimuth_angle, solar_zenithal_angle, angle_view_zenith, angle_view_azimuth, theta_s_out):
    ...
    add_node_xml(newdoc, root, 'ACCODE', "LEDAPS version 3.4.0")

    for i in range(6):
        add_node_xml(newdoc, root, "MSI_BAND_{}_BANDPASS_ADJUSTMENT_SLOPE".format(table_band_adjustment[i][0]), str(table_band_adjustment[i][2]))
        add_node_xml(newdoc, root, "MSI_BAND_{}_BANDPASS_ADJUSTMENT_OFFSET".format(table_band_adjustment[i][0]), str(table_band_adjustment[i][1]))
...
list_image_process = os.listdir(input_LEDAPS_result)
compte = 1
for file in list_image_process:
    print("File {}/{} : {}".format(compte, len(list_image_process), file))
    compte += 1
    t = time.time()
    #Process each image in input_LEDAPS_result file
    directory_LaSRC = os.path.join(input_LEDAPS_result, file)
    ...
    dst_transform = coregistration(directory_LaSRC, directory_workspace, translation_latitude, translation_longitude, file)
    ...
    #Process band adjustment
    print("Processing band adjustment...")

    print("Band adjustment processed")
    print("")
